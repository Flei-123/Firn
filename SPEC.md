...
crypto library also run in the `app` profile, but use **no** GC types and are
...
therefore never reached them. `modules::build_program` now also sends the
...
S8. **Literals lie in the frame, not in `.rodata`.** It follows from S1 that the
    ...
    Also open: `Str`/`Bytes`/`Str16` as the **type** of a literal -- today the
