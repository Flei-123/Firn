...
| **What is missing for `[x]`** | (a) the **24 hour run**, (b) **fragmentation with changing object sizes** -- the soak test always uses the same set, which is the friendly case, (c) `virtual`, and, for the collections, the **nominal type safety of the container** (`docs/ROUND53.md` 4.1). Incremental collection (round 44), finalizers (round 47) and `GcVec`/`GcMap` (round 53) are done |
...
| **Status after round 48** | **`[~]` further along, the criterion still not satisfied.** There is now a **project system**: the manifest `firn.pkg` (name, version, entry point, source directories, public modules, local dependencies), a fixed search order (own file -> root file -> project sources -> dependencies -> `$FIRNLIB` -> `<exe>/../lib`), **visibility at module level** as the package interface, detection of package cycles and of module name clashes, and the build driver `firnc --package <dir>` -- all of it in **both** compilers with character-identical messages (`compiler/src/package.rs`, `compiler/src/package_world.rs`, `lib/firnc1/package.fi`; `tools/packages/run.sh`: 21 cases, `demos/packages/`). **Still not present:** network/registry, a lock file with checksums, version resolution, `firn build --locked`, a two-machine comparison via `sha256sum`. The criterion of this item therefore remains **not** satisfied (`docs/ROUND48.md`) |
| **Status after round FIRNHUB (measured in person, 30.08.2026, branch `firnhub`)** | **`[x]` holds, and it now covers a dependency that does not lie on this machine.** `needs` takes a git source with a fixed reference, an archive with a `#sha256=` or a version out of an index; the kind is decided purely lexically, so both compilers classify the same text alike. **The compiler never speaks to the network**: `firnpkg fetch` (in Firn, once -- the doubling rule is worth its price for a format, not for `git clone`) resolves a source, weighs an archive against its checksum BEFORE `tar` sees it, hashes the unpacked tree and puts it under `<cache>/pkg/<content hash>`; `firn.have` next to the root manifest carries name, source, resolved reference and content hash for the whole graph, and from there on the compiler sees local paths only -- which is why an offline build needs no switch. `firn.lock` **format 2**: a fetched package is addressed as `cache:<hash>` (a path would mean nothing on the second machine) plus one `origin` line per fetched package. **Measured** (`tools/hub/run.sh`, 37 checks, 0 failed): a real git repository with a real tag, fetched, built by BOTH compilers, both writing a character identical `firn.lock`; twice fetching and twice building change nothing octet for octet; a file edited INSIDE the cache is caught twice (`firnpkg verify` and `--locked`, same sentence from both compilers); a re-pointed git tag is refused (trust on first use); the whole archive path including an archive with the wrong checksum. Two real libraries as the proof (`demos/hub/json` 1.0.0, `demos/hub/date` 1.1.0, each with its own tests as its `main`). **Not claimed:** no signatures (`docs/ROUND-FIRNHUB.md` names what Ed25519 in Firn would cost -- about 400 lines, and twice if it ever moves into the compiler), no index over HTTPS, no cache clean-up, and still no separate compilation |
...
| 5 | Package management reproducible | `[x]` project system (round 48), lock file with a two-machine proof (round 93) and **remote sources** (round FIRNHUB: `git+<url>#<ref>`, an archive with `#sha256=`, a version out of an index; `firnpkg fetch` into a content addressed cache, `firn.have`, `firn.lock` format 2 with `cache:<hash>` and `origin` lines) -- all of it in both compilers with octet identical output. **What is deliberately NOT claimed: nothing is signed** (no Ed25519 in Firn; the price is named in `docs/ROUND-FIRNHUB.md`), and there is no service that hands out names |
