...
| 5 | Package management reproducible | `[~]` module system **and project system** present (round 48: manifest `firn.pkg`, dependencies via local paths, visibility at module level, build driver `--package`, in both compilers); **registry, lock file and the two-machine proof are missing** |
