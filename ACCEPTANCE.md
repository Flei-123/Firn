...
**State of this file:** 2026-08-26, **after round 96**
**Overall result: 2 of 6 passed** (items 5 and 6), 3 partial (items 2, 3, 4),
1 open (item 1).

**Round 96 (2026-08-26, measured in person, branch `r96`)** closed two of the
three remainders it was given and corrected the third:

* **the constant-time item**: `secret[T]` is a type now, with the whole rule
  set of SPEC 9.1 behind it. `bash tools/ct/run.sh` -> **65 checks, 0
  failed**: 23 programs refused with line and column, the assembly of a
  comparison of two secrets free of any conditional jump of control flow at
  all four build levels, and the public twins of the same functions carrying
  one each. `docs/ROUND96.md`
* **the debugger**: variables are visible in an OPTIMIZED build for the first
  time (`--debug-vars`, DWARF location lists in `.debug_loc`).
  `bash tools/dwarf/run.sh` -> **73 checks** (was 63): at `dev-fast` **8 of
  11** named variables of `tools/dwarf/probe.fi` carry a location, at the two
  release levels **6 of 11** -- before the round it was **0** at every
  optimized level, and **not one value gdb printed was wrong**
* **the package item**: the summary table below said registry, lock file and
  the two-machine proof were missing. Two of the three have existed since
  round 93; re-measured in person for this round: `tools/packages/run.sh`
  **39 of 39**, `tools/repro/two_machines.sh` **PASS**. The registry is the
  one that is really missing, and the criterion of the item does not ask for
  one. The table is corrected below

**State of the suite on 2026-08-26 (round 96, run in person):** `bash test.sh`
sections 1-17 of 63 green, **0 failures** -- 333 programs x 4 build levels =
**1,332 checks**, **211 negative tests**, the section proofs of the optimizer,
the result location, the architecture guards, the symbol scheme, the atomic
primitive, the interface bounds and the function values, the HTML5 tokenizer
**6,810 / 6,810**, the four comparisons against `firnc1`, `self_compare`
**332 same / 0 differing**, and the fixpoint **stage 2 == stage 3,
character-identical** (781,717 lines of assembly, 4,657,448 octets each). The
remaining sections were still running when this was written; this machine
shares its cores with another acceptance run. On top of that, run
individually and in person: `tools/ct/run.sh` **65/65**,
`tools/dwarf/run.sh` **73/73**, `tools/packages/run.sh` **39/39**,
`tools/repro/two_machines.sh` **PASS**, `cargo test --release` **257 passed**,
`tools/english/check.sh` **0 German identifiers, 0 text sites, 0 wrong length
values**, `firnfmt -c` clean on every file this round touched.
...
| **Measured in round 96 (2026-08-26, in person)** | `bash tools/fixpoint.sh` -- the script that does what the criterion describes, under another name: `firnc0` compiles `bin/firnc1.fi` to `.firnc1`, `.firnc1` compiles the same source to `.firnc2`, `.firnc2` compiles it to `.firnc3`. **STAGE 2 == STAGE 3, character-identical, 781,717 lines of assembly, 4,657,448 octets each**, and `.firnc2` then runs over the whole test corpus and behaves like `firnc0`. The row above is out of date by a long way; whether that is enough for the tick belongs to whoever owns this item, and round 96 only writes down what it ran |
...
| **Status B after round 96 (measured in person, 26.08.2026, branch `r96`)** | **`[~]` the variable half is no longer open, and it is not finished either.** What round 94 named as the reason is what got built: `dwarf::VarPlace` gives the three answers that can be given TRUTHFULLY -- the `alloca` still has a frame address (`DW_OP_fbreg`), the register allocator holds the cell in a register (`DW_OP_reg<N>`, `regalloc::promotable_cells` keeps one register per function), or the storage is gone and `mem2reg` left a **trail**, in which case the variable gets a **LOCATION LIST** in `.debug_loc` that starts at the label where the value comes into being. Everything that cannot be answered for gets no entry, and `gdb` says `optimized out` -- a wrong value in a debugger is worse than none. The trail is written only where it has ONE answer (a cell written exactly once: every parameter, every `let`); a cell written in a loop has a different value per block and gets nothing. Switched on with `--debug-vars` (`-g`), off by default because debug information carries the working directory and a build with it is not reproducible octet for octet (round 93). **Measured** (`bash tools/dwarf/run.sh`, section 9, now **73 checks**, over the 11 declared names of `tools/dwarf/probe.fi`): `dev` **11 of 11** variables with a location, `dev-fast` **8** (3 of them location lists), `release-safe` and `release-fast` **6** (1 list) -- before the round it was **0** at every optimized level. The values are held against what the program really computes (`shift(&p,3)` makes `p = {8, 10}` and returns 18, `total(2)` computes 12): **4 of 4 right at `dev`, 4 of 4 at `dev-fast`, 2 of 4 at the release levels, 0 WRONG anywhere**. Counter-checks: an optimized build WITHOUT `-g` carries 0 variables, and a deliberately wrong expectation strikes. **Still open:** a variable written more than once (needs the trail through the phi nodes of round 92), and `aarch64` (the places come out of the x86 register allocator) |
| **`a real bug was found with it` after round 96** | **`[x]` done, and the bug is fixed.** The method the new variable information makes possible (`tools/dwarf/diff_levels.sh`): the same file compiled twice, `--no-opt -g` against `--opt-level=dev-fast -g`, stopped at the same function in both, and the PARAMETERS held against each other -- a parameter has an exactly defined value at a function entry, the one the caller passed. It reported `val__realm_bool(on)` of `tests/1002_js_interp.fi` as `false` without the optimizer and `true` with it. **Neither value was wrong: the BREAKPOINT was in a different place.** `break <function>` has to skip the prologue, and when the line table does not say where the prologue ends, `gdb` guesses -- it takes the second line entry of the function, and with the optimizer that lands BEHIND the first conditional branch. The breakpoint sat inside one arm of the `if` and was hit for only half the calls; the backtraces name two different call sites. The cause, exactly: `fir::Term` carries no source position (round 94 put the position on the INSTRUCTIONS), so a function whose body begins with a branch has nothing left in its entry block once `mem2reg` has taken the load of the parameter away -- not one `.loc` announces the body. **Fixed** with `.loc … prologue_end`, which DWARF has for this and GNU `as` writes: on the first instruction of the body that has a position of its own (in front of it lie the stores that put the parameters where the debug information says they are -- a marker at the frame setup made `docs/gdb_example.fi` answer `n = 0` where the caller passed 10, which is how the second trap was found), and right after the frame setup where the entry block has no position at all. After the fix both builds stop at the same call with the same value, and `tools/dwarf/run.sh` stays at **73 of 73** -- the marker moves no breakpoint that was already right. What is NOT claimed: the terminator still has no position, so stepping through a condition at an optimized level still shows the `fn` line (`docs/ROUND96.md` 7) |
| **Item 4 overall** | **`[~]`** -- **A satisfied** (test runner over test FUNCTIONS, JSON/TAP, crash-isolated, 35 of 35 measured), **B satisfied**: lines right at every build level (round 94), variables in an optimized build for the first time (round 96, 8 of 11 at `dev-fast`), and a real bug found with the debugger AND fixed (round 96) |
...
| **Re-measured in round 96 (2026-08-26, in person, branch `r96`)** | the summary table of this file still said "registry, lock file and the two-machine proof are missing" -- a sentence from round 3 that round 93 had made false and nobody struck out. Both proofs were run again before a line of this round was written: `bash tools/packages/run.sh` -> **39 of 39**, `bash tools/repro/two_machines.sh` -> **PASS**, same artifact out of both compilers on both machines, same `firn.lock` out of all four runs. **What is really missing is the registry**, and the criterion of this item does not ask for one -- it asks for the bit-identical artifact. Round 96 did not build a registry: it would have to be built in BOTH compilers with octet-identical messages, like the rest of the package system, and half of it would be worth less than none |
...
| # | Item | Status after round 96 (2026-08-26, measured in person) |
|---|---|---|
...
| 4 | Test runner **and** debugger | `[~]` **A satisfied** -- round 94 makes a test a FUNCTION (`#[test]`, `firnc --test`), JSON/TAP with name, outcome, duration and file:line:column, one process per case, `tools/testrunner/run.sh` **35/35**. **B:** lines are right at **every** build level and are held against the program's own panic message (`tools/dwarf/run.sh` **73/73**, lines covered 10/7/5/5 instead of 10/1/1/1). **Round 96: the location lists are built** -- variables in an optimized build for the first time (`--debug-vars`, `.debug_loc`), **8 of 11 at `dev-fast`, 6 of 11 at the release levels, 0 of 11 before**, and no value gdb printed was wrong. **Round 96 also delivers "a real bug was found with it"**: a breakpoint on a function landed behind the first conditional branch at every optimized level and was hit for half the calls -- found by comparing the two build levels through the debugger, fixed with `.loc prologue_end`. Open: a variable written more than once, and `aarch64` |
| 5 | Package management reproducible | `[x]` **since round 93, re-measured in round 96**: module system and project system (round 48), the lock file `firn.lock` with SHA-256 over manifest and sources, `--lock`/`--locked`, and the two-machine proof -- `tools/repro/two_machines.sh` **PASS** (another working directory, `$HOME`, `$TMPDIR`, `$TZ`, `$PATH`, umask, wall clock, source time stamps, creation order, compiler path, and the second run under `qemu-x86_64`), `tools/packages/run.sh` **39/39**. **Not claimed:** network and registry -- `needs` knows local paths |
| 6 | Compile-time codegen produces the UCD table | `[x]` **since round 95** (`generated/unicode_tables.fi` out of the real UCD, 23,888 octets, checked against all 1,114,112 code points, reproducible octet for octet). The row below is the state of round 3 and is kept for the history: `[~]` **`comptime` built (round 12)**: own functions run at compile time, with loops, branches and recursion (`compiler/src/comptime.rs`, `tests/600_comptime.fi`). **`emit` built (round 13)**: `comptime` blocks produce Firn source text that is lexed, parsed and compiled in the same run (`tests/601_comptime_emit.fi`, `firnc --emit=comptime`). **Data access built (round 14)**: `datei_groesse`/`datei_byte` read at compile time; `tests/602_comptime_ucd.fi` produces `ucd_gross` from a file in `UnicodeData.txt` format. **In substance the item is thereby satisfied** -- what is missing is proving it against the *real* UCD (1.9 MB, all categories) and a build script that fetches it. Access is restricted to the directory of the source file (no `..`, no absolute path) |

**Overall after round 96: 2 of 6 passed** (5, 6), **3 partial** (2, 3, 4),
**1 open** (1 -- and the fixpoint of round 31 has been running for sixty
rounds; see the measured row under item 1).

**Overall after round 3 (the state this table was written in): 0 of 6 passed,
4 started** (2, 3, 4, 5).
...
| 6 | Constant time (`secret[T]`, `select`, `secure_zero`, `u128`) | **`[x]` since round 96** -- `secret[T]` is a type; `u128`/`mul_wide` are named as open | Round 4 built the three primitives (`select` -> `cmov`, `barrier` survives constant folding, `secure_zero` survives the optimizer) and left the qualifier itself out; without it there was no type check for secret data at all. **Round 96 (2026-08-26, in person, branch `r96`)**: `types::Type::Secret` is a variant of its own and deliberately NOT transparent -- every `is_concrete_int()` in the compiler says NO to a secret, so a place that has not been taught about them REFUSES them (that is how printing a secret, `match` on a secret and a `const` of one came to be refused without a rule of their own). The rules of SPEC 9.1 live in `ct.rs`: no branch (`if`, `while`, `&&`, `\|\|`, `match`), no index (`C4`), no division, **no checked or saturating arithmetic** -- their overflow test IS a conditional jump on secret data, which is the find of the round -- no secret shift amount, and no way out with `as`. Allowed: the bit operations, the explicit wrapping forms, a shift by a public amount, and the comparisons, which yield `secret[bool]`. `declassify(x)` is the one way out and becomes a `Barrier` in the IR. The marking travels into the IR at ONE place (`lower::lower_expr` -> `Func::secret`), where the passes of round 4 have been waiting for it, and `#[constant_time]` switches the check in the code generator on (`attrs.rs`: implemented). **Measured:** `bash tools/ct/run.sh` -> **65 checks, 0 failed** -- **20 new negative tests** with line and column, the assembly of `ct_eq`/`ct_choose`/`ct_mask`/`ct_eq4` free of any conditional jump of control flow at **all four build levels** while `public_eq`/`public_choose` carry one each, `select` on a secret condition really a `cmov`, `secure_zero` surviving at every level, and four positive programs (`tests/434`-`437`) that run and answer the same at all four levels. **Our own AES answered the question its note A3 asked**: `tools/ct/aes_probe.fi` (SubBytes with the state declared as key material) is refused at `25:29` and `31:17`; the constant-time way (`tools/ct/aes_ct.fi`, `select` over all 256 entries) gives the same answer for all 256 inputs and costs **3 ns -> 1,595-1,816 ns per substitution**. **Deliberately still open and named:** `u128`/`i128` and `mul_wide` (SPEC 9.3, `C5`), and `secret[T]` in `firnc1` -- the compiler in Firn counts a file that uses it as NOT CORE LANGUAGE by spelling (as it does the SIMD words of round 82) and **accepts none** of the 23 files `firnc0` refuses |
