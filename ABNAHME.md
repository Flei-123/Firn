...
| **Was fehlt bis `[x]`** | (a) der **24-Stunden-Lauf**, (b) **Fragmentierung bei wechselnden Objektgrößen** — der Dauerlauf benutzt immer denselben Satz, das ist der freundliche Fall, (c) `GcVec`/`GcMap`, `virtual`. Inkrementelles Sammeln (Runde 44) und Finalisierer (Runde 47) sind erledigt |
...
| 5 | Paketverwaltung reproduzierbar | `[~]` Modulsystem **und Projektsystem** vorhanden (Runde 48: Manifest `firn.paket`, Abhängigkeiten über lokale Pfade, Sichtbarkeit auf Modulebene, Bau-Treiber `--paket`, in beiden Übersetzern); **Registry, Sperrdatei und Zwei-Rechner-Nachweis fehlen** |
| 6 | Kompilierzeit-Codegen erzeugt UCD-Tabelle | `[~]` **`comptime` gebaut (Runde 12)**: eigene Funktionen laufen zur Übersetzungszeit, mit Schleifen, Verzweigungen und Rekursion (`compiler/src/comptime.rs`, `tests/600_comptime.fi`). **`emit` gebaut (Runde 13)**: `comptime`-Blöcke erzeugen Firn-Quelltext, der im selben Lauf gelext, geparst und übersetzt wird (`tests/601_comptime_emit.fi`, `firnc --emit=comptime`). **Datenzugriff gebaut (Runde 14)**: `datei_groesse`/`datei_byte` lesen zur Übersetzungszeit; `tests/602_comptime_ucd.fi` erzeugt `ucd_gross` aus einer Datei im `UnicodeData.txt`-Format. **Der Punkt ist damit inhaltlich erfüllt** — was fehlt, ist die Bewährung an der *echten* UCD (1,9 MB, alle Kategorien) und ein Bauskript, das sie holt. Zugriff ist auf das Verzeichnis der Quelldatei beschränkt (kein `..`, kein absoluter Pfad) |
