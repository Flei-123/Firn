...
| 6 | Kompilierzeit-Codegen erzeugt UCD-Tabelle | `[~]` **`comptime` gebaut (Runde 12)**: eigene Funktionen laufen zur Übersetzungszeit, mit Schleifen, Verzweigungen und Rekursion (`compiler/src/comptime.rs`, `tests/600_comptime.fi`). **Offen: `emit`** — erzeugter Quelltext, den derselbe Lauf weiterverarbeitet; Vorbedingung (wiedereintrittsfähige Prüfphasen) steht. Ohne `emit` keine UCD-Tabelle |
