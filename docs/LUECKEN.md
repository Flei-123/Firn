...
| B4 | closed on branch `runde-b4-barrier`, see `docs/ROUND-B4.md`: the fast half of the barrier inline at every store (dev-fast 11.6 -> 2.9 ns per store), slow half `__gc_barrier_slow` only during a cycle | | | | | |
