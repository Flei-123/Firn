...
| LIB-005 | `fuishell.dock`, `fuishell.filechooser` (`lib/fuishell/`), `window.wait_any`, `fuiwin.fenster_step`/`fenster_wait_many` | **in part**: the dock model (areas left/right/bottom/center with tabs, splitter and tab dragging, layout saved/loaded as JSON), `Split`, the file chooser model (listing, sort, glob filter, open/save/directory), and several top-level windows in one program. The dock is also drawn (`fuishell.dockpaint`: tab strips, active tab with accent line, ellipsized titles, splitters, the drop hint while a tab is dragged). Still open: drawing the file chooser with fUi widgets and wiring its input | `tests/1918`, `tests/1919`, `tests/1920` (pixels read back); two windows on an Xvfb driven by python-xlib |
