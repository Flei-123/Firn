...
# tools/promote/run.sh -- round OPT-GENERAL: the promotion pass under stress.
#
# `promote` (compiler/src/promote.rs) rotates only the loops that pay off.
# This runs a set of programs in both release levels, once normally and once
# in the stress mode that rotates EVERY loop (FIRN_PROMOTE_ROTATE_ALL=1
# FIRN_PROMOTE_CLEANUP_FIRST=1), against the expectation in line 1 of each
# file -- the stress mode is what found the rotation bug of this round. The programs: the pass's own test,
...
for f in $PROGS; do
    ...
    for lvl in release-fast release-safe; do
        for mode in normal stress; do
            ...
            if [ $mode = stress ]; then
                env FIRN_PROMOTE_ROTATE_ALL=1 FIRN_PROMOTE_CLEANUP_FIRST=1 \
...
echo "   promote (normal + stress): $pass passed, $fail failed"
[ "$fail" -eq 0 ]
