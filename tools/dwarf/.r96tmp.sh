...
#      does not have.
#   9. ROUND 96 -- VARIABLES IN AN OPTIMIZED BUILD. `--debug-vars` (`-g`)
#      asks for names, types and PLACES at every build level; the places come
#      out of the register allocator and out of the trail `mem2reg` leaves
#      behind when it takes a variable's storage away (`.debug_loc`, a
#      location list per variable that only exists from its definition on).
#      Measured per level: how many variables carry a location at all, and --
#      the number that decides -- whether any of them shows a WRONG value.
#      Missing is the price of the optimization and is counted; wrong is a
#      failure.
#   6. counter-checks. Without them the whole thing would be worthless:
#      * an optimized build WITHOUT `-g` must carry NO variable information
#        -- a wrong value in the debugger is worse than none
#        (docs/DEBUGGER.md)
...
echo
echo "== 9. ROUND 96: variables in an OPTIMIZED build (--debug-vars) =="
# The reference is the program itself: `shift(&p, 3)` makes p = {8, 10} and
# returns 18, `total(2)` computes 1+2+3+4+2 = 12, and `flag` is true because
# 18 > 10. Those are the values gdb has to print -- or say nothing at all.
for LVL in dev dev-fast release-safe release-fast; do
    ...
    rm -f "$TMPD/vp_$LVL"
    if ! "$FIRNC" $FLAG -g -o "$TMPD/vp_$LVL" tools/dwarf/probe.fi > "$TMPD/vp_build.log" 2>&1; then
        bad "$LVL: probe.fi does not build with --debug-vars"
        continue
    fi
    dies=$(readelf --debug-dump=info "$TMPD/vp_$LVL" 2>/dev/null |
        grep -cE 'DW_TAG_(variable|formal_parameter)')
    withloc=$(readelf --debug-dump=info "$TMPD/vp_$LVL" 2>/dev/null | grep -c 'DW_AT_location')
    lists=$(readelf --debug-dump=loc "$TMPD/vp_$LVL" 2>/dev/null | grep -c 'End of list')
    gdb -batch -ex "break tools/dwarf/probe.fi:35" -ex run \
        -ex "info locals" -ex "info args" -ex kill \
        "$TMPD/vp_$LVL" > "$TMPD/vg_$LVL.txt" 2>&1
    python3 tools/dwarf/vars_check.py "$TMPD/vg_$LVL.txt" \
        "p={x = 8, y = 10}" "moved=18" "flag=true" "sum=12" > "$TMPD/vc_$LVL.txt" 2>&1
    res=$(tail -1 "$TMPD/vc_$LVL.txt")
    wrong=$(echo "$res" | sed 's/.*wrong=\([0-9]*\).*/\1/')
    checked=$(echo "$res" | sed 's/.*checked=\([0-9]*\).*/\1/')
    echo "   $LVL: $dies variable DIEs, $withloc with a location, $lists location list(s), $res"
    if [ "$wrong" -eq 0 ]; then
        ok
    else
        bad "$LVL: gdb shows a WRONG variable value"
        sed 's/^/        /' "$TMPD/vc_$LVL.txt"
    fi
    # The measurement must not be empty: at least one variable has to be
    # readable, otherwise "no wrong value" would be true of nothing.
    if [ "$checked" -ge 1 ]; then
        ok
    else
        bad "$LVL: not one variable was readable -- the check measures nothing"
    fi
done
# The two counter-checks of this section. Without them the numbers above
# would prove nothing.
# 9a. A deliberately wrong expectation has to strike.
python3 tools/dwarf/vars_check.py "$TMPD/vg_dev.txt" "moved=999" > "$TMPD/vc_wrong.txt" 2>&1
if grep -q "wrong=1" "$TMPD/vc_wrong.txt"; then
    ok
else
    bad "counter-check: a wrong expectation (moved=999) does not strike"
fi
# 9b. WITHOUT `-g` an optimized build stays free of variables -- the flag has
# to be what decides, not the round.
rm -f "$TMPD/vp_nog"
"$FIRNC" --opt-level=dev-fast -o "$TMPD/vp_nog" tools/dwarf/probe.fi > /dev/null 2>&1
n=$(readelf --debug-dump=info "$TMPD/vp_nog" 2>/dev/null | grep -c 'DW_TAG_variable')
if [ "$n" -eq 0 ]; then
    ok
    echo "   without -g: $n variables, as before the round"
else
    bad "an optimized build without -g carries $n variables"
fi

echo "== 6. counter-checks =="
# 6a. WITH the optimizer and WITHOUT -g there must be no variable information.
