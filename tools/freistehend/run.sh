#!/usr/bin/env bash
# tools/freistehend/run.sh — DER NACHWEIS, DASS `profile kernel` ETWAS BEDEUTET.
#
# Runde 52. Geprueft wird, was man an der erzeugten Datei ABLESEN kann, nicht
# was der Compiler ueber sich selbst behauptet:
#
#   1. `beispiele/kernel/kern.fi` uebersetzt mit BEIDEN Compilern zu einer
...
#   4. Sie laesst sich mit `ld -T beispiele/kernel/linker.ld` zu einem Abbild
...
FC1=${FIRNC1:-./.firnc1}
QUELLE=beispiele/kernel/kern.fi
SKRIPT=beispiele/kernel/linker.ld
...
as --64 -o "$TMPD/start.o" beispiele/kernel/start.s 2>"$TMPD/as.err" \
...
# Die SCHARFE Zaehlung: tools/freistehend/volatile.fi hat alles in EINER
...
VOL=tools/freistehend/volatile.fi
for stufe in "" "--no-opt" "--opt-level=dev-fast"; do
    name=${stufe:---release-fast}
