...
/// Faden erzeugen.
pub(crate) const START: &str = "__faden_starten";
/// Eigener Fadenblock (TLS, `fs:0`).
pub(crate) const SELBST: &str = "__faden_selbst";
/// Atomarer Vergleichs-Tausch.
pub(crate) const CAS: &str = "__atomar_tauschen";
...
pub(crate) const EINSTIEG: &str = "__faden_einstieg";
...
pub(crate) fn ist_faden_call(name: &str) -> bool {
...
pub(crate) fn hook_call(
    ...
    if !ist_faden_call(name) || ck.fns.contains_key(name) {
        return None;
    }
    let _ = nspan;
    match name {
        ...
        START => {
            ...
            if !at.is_error() && !passt_als_u64(&at) {
                ck.dg.error(
                    args[0].span,
            ...
            if !st.is_error() && !passt_als_u64(&st) {
                ck.dg.error_note(
                    args[1].span,
                    format!(
        ...
        _ => {
            ...
            if !pt.is_error() && !ist_u64_zeiger(&pt) {
                ck.dg.error_note(
                    args[0].span,
                    format!(
...
pub(crate) fn lower_faden_call(
    ...
    match name {
        ...
        START => {
            ...
            Some(Some(lo.push(FTy::I64, Op::ThreadSpawn { arg: a, stapel: s, ctid: c })))
        }
        _ => {
            if args.len() != 3 {
            ...
            Some(Some(lo.push(FTy::U64, Op::AtomicCas { addr: p, erw: e, neu: n })))
        }
    }
}
...
pub(crate) fn spawn_sequenz(e: &mut crate::codegen_x86::Emitter) {
...
pub(crate) fn cas_sequenz(e: &mut crate::codegen_x86::Emitter) {
...
pub(crate) fn selbst_sequenz(e: &mut crate::codegen_x86::Emitter) {
