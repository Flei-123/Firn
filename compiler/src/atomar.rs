...
//! (`tools/atomar/run.sh` liest den Assembler und verlangt das `lock`-Praefix).
...
pub(crate) const ADD: &str = "__atomar_addieren";

/// Ist `name` der Name des eingebauten Primitivs?
pub(crate) fn is_atomar_call(name: &str) -> bool {
    name == ADD
}
...
pub(crate) fn hook_call(
    ...
    if !is_atomar_call(name) || ck.fns.contains_key(name) {
        return None;
    }
    if args.len() != 2 {
        for a in args {
    ...
    if !dt.is_error() && !passt_als_u64(&dt) {
        ck.dg.error(
            args[1].span,
            format!(
...
pub(crate) fn lower_atomar_call(lo: &mut Lower, name: &str, args: &[Expr], span: Span) -> Option<Option<Val>> {
    let _ = name;
    if args.len() != 2 {
