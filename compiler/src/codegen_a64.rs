...
pub fn emit(m: &Module) -> Result<String, String> {
    ...
    // ROUND 87: `crc32cb`/`crc32cx` (the counterpart of SSE 4.2's `crc32`)
    // belong to the optional CRC extension of ARMv8-A. The assembler has to
    // be told, otherwise it refuses the mnemonic even where the instruction
    // is never reached.
    e.raw(".arch armv8-a+crc");
    e.raw(".text");
    e.raw(".globl _start");
    e.raw("_start:");
