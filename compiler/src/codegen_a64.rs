...
pub fn emit(m: &Module) -> Result<String, String> {
    ...
    let mut e = Emitter { out: String::new(), xmm: Default::default(), debug_funcs: Vec::new() };
    e.raw(&format!(
...
fn emit_inst(e: &mut Emitter, f: &Func, fr: &Frame, i: &Inst) -> Result<(), String> {
    let ty = i.ty;
    match &i.op {
        // ROUND 82 on ROUND 80: the processor's own vector and crypto
        // instructions exist for x86-64 only. aarch64 has its counterparts
        // (`aese`/`aesmc`, `sha256h`), they are simply not emitted yet. Say
        // so and stop -- a silent wrong result would be far worse.
        Op::Simd { kind, .. } => {
            return Err(format!(
                "--target=aarch64-linux cannot emit the vector instruction {:?} yet (round 82 built them for x86-64; \
                 the scalar path of lib/std/crypto works on both machines)",
                kind
            ));
        }
        Op::Const(c) => {
...
mod tests {
    ...
    fn large_constants_come_out_as_movz_movk() {
        ...
        let mut e2 = Emitter { out: String::new(), xmm: Default::default(), debug_funcs: Vec::new() };
        imm_into(&mut e2, "x9", -1);
        assert_eq!(e2.out.trim(), "movn x9, #0", "{}", e2.out);
        let mut e3 = Emitter { out: String::new(), xmm: Default::default(), debug_funcs: Vec::new() };
