...
| `demos/webdemo/DejaVuSans.ttf` | DejaVu Sans 2.37 unchanged (Debian `fonts-dejavu-core` 2.37-6) -- the font `tools/fui/run.sh` paints its reference PNGs with, so the browser demo can be compared pixel for pixel | Bitstream Vera licence, full text in `LICENSES/Bitstream-Vera.txt` |

Provenance, versions and checksums are documented in `tools/ucd/SOURCE.md`,
`tests/data/fonts/PROVENANCE.md` and `demos/webdemo/README.md`.
